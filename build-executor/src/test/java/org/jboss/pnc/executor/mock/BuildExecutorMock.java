...
public class BuildExecutorMock implements BuildExecutor {
    ...
    private Map<Integer, BuildExecutionConfiguration> runningExecutions = new HashMap<>();

    private ExecutorService executor = Executors.newFixedThreadPool(4);

    @Override
    public BuildExecutionConfiguration build(BuildConfiguration buildConfiguration, BuildConfigurationAudited buildConfigAudited, User user, Consumer<BuildCoordinationStatus> onComplete, Set<Integer> buildRecordSetIds, Integer buildConfigSetRecordId, Integer buildTaskId, Date submitTime) throws CoreException {
        ...
        startBuilding(buildExecutionConfiguration, onComplete);

        return buildExecutionConfiguration;
    }

    @Override
    public void startBuilding(BuildExecutionConfiguration buildExecutionConfiguration, Consumer<BuildCoordinationStatus> onComplete) throws CoreException {
        runningExecutions.put(buildExecutionConfiguration.getId(), buildExecutionConfiguration);
        Consumer<BuildCoordinationStatus> onCompleteInternal = (buildStatus) -> {
    ...
    private Integer complete(Consumer<BuildCoordinationStatus> onCompleteInternal) {
        onCompleteInternal.accept(BuildCoordinationStatus.DONE);
        return -1;
    }
