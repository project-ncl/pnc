...
public class WeightedGraphBuilder<T, S> {
    ...
    public Vertex<T> buildDependencyGraph(Graph<T> graph, S nodeId, int depthLimit) {
        return buildGraph(graph, nodeId, depthLimit, this::getDependencies, (vertex, dependencyVertex, cost) -> {
            log.trace("Creating new dependency edge from {} to {}.", vertex, dependencyVertex);
            graph.addEdge(vertex, dependencyVertex, cost);
        });
    }

    public Vertex<T> buildDependantGraph(Graph<T> graph, S nodeId, int depthLimit) {
        return buildGraph(graph, nodeId, depthLimit, this::getDependants, (vertex, dependantVertex, cost) -> {
            log.trace("Creating new dependant edge from {} to {}.", dependantVertex, vertex);
            graph.addEdge(dependantVertex, vertex, cost);
        });
    }

    private Vertex<T> buildGraph(
            Graph<T> graph,
            S nodeId,
            int depthLimit,
        ...
        for (VertexNeighbor<S> vertexNeighbor : neighborSupplier.apply(node)) {
            ...
            if (neighborVertex == null) {
                neighborVertex = buildGraph(graph, neighborId, depthLimit - 1, neighborSupplier, edgeAdder);
            }
