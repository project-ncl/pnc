...
public class SecondLevelCacheDebugTest extends AbstractModelTest {
    ...
    public void testBuildConfigurationInsertion() throws Exception {
        ...
        assertTrue(bc.getDependencies().stream().map(dependencyConfig -> dependencyConfig.getId()).collect(Collectors.toSet())
    ...
    public void testBogusBuildConfigurationUpdate() {
        ...
        BuildConfiguration bc = session_1.load(BuildConfiguration.class, buildConfigurationId);
        RepositoryConfiguration rc = session_1.load(RepositoryConfiguration.class, bc.getRepositoryConfiguration().getId());
        ...
        BuildConfiguration.Builder bcBuilder = BuildConfiguration.Builder.newBuilder().id(bc.getId()).name(bc.getName())
                .description(bc.getDescription()).buildScript(bc.getBuildScript()).scmRevision(bc.getScmRevision())
                .archived(bc.isArchived()).genericParameters(bc.getGenericParameters()).buildType(bc.getBuildType());
    ...
    private void rebuildRepositoryConfiguration(RepositoryConfiguration rc, BuildConfiguration.Builder bcBuilder) {
        bcBuilder.repositoryConfiguration(RepositoryConfiguration.Builder.newBuilder().id(rc.getId())
                .internalUrl(rc.getInternalUrl()).externalUrl(rc.getExternalUrl())
                .preBuildSyncEnabled(Boolean.TRUE.equals(rc.isPreBuildSyncEnabled())).build());
    }

    private void rebuildProject(Project p, BuildConfiguration.Builder bcBuilder) {
        Project.Builder pBuilder = Project.Builder.newBuilder().id(p.getId()).name(p.getName()).description(p.getDescription())
                .issueTrackerUrl(p.getIssueTrackerUrl()).projectUrl(p.getProjectUrl());
        List<Integer> buildConfigurationIds = nullableStreamOf(p.getBuildConfigurations())
                .map(buildConfig -> buildConfig.getId()).collect(Collectors.toList());
        nullableStreamOf(buildConfigurationIds).forEach(buildConfigurationId -> pBuilder
                .buildConfiguration(BuildConfiguration.Builder.newBuilder().id(buildConfigurationId).build()));
        if (p.getLicense() != null) {
            pBuilder.license(License.Builder.newBuilder().id(p.getLicense().getId()).build());
        }
    ...
    private void rebuildBuildEnvironment(BuildEnvironment bE, BuildConfiguration.Builder bcBuilder) {
        bcBuilder.buildEnvironment(BuildEnvironment.Builder.newBuilder().id(bE.getId()).name(bE.getName())
                .description(bE.getDescription()).systemImageRepositoryUrl(bE.getSystemImageRepositoryUrl())
                .systemImageId(bE.getSystemImageId()).attributes(bE.getAttributes()).systemImageType(bE.getSystemImageType())
                .deprecated(bE.isDeprecated()).build());
    }
    ...
    private void rebuildDependencies(BuildConfiguration bc, BuildConfiguration.Builder bcBuilder) {
        Set<Integer> dependencyIds = nullableStreamOf(bc.getDependencies()).map(dependencyConfig -> dependencyConfig.getId())
