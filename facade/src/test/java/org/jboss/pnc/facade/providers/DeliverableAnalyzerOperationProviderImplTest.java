...
import org.jboss.pnc.facade.providers.AbstractProviderTest.ListAnswer;
...
public class DeliverableAnalyzerOperationProviderImplTest
    ...
    public void testSaveOrUpdate() {
        ...
        org.jboss.pnc.dto.DeliverableAnalyzerOperation operationDTO = delAnalyzerOperationMapper.toDTO(operation);

        // Store operation and verify it exists in DB
        operationDTO = provider.saveOrUpdate(operationDTO.getId(), operationDTO);
        ...
        assertThat(all.getContent()).hasSize(1)
                .haveExactly(
                        1,
                        new Condition<>(op -> operation.getId().getId().equals(op.getId()), "Operation present"));
        org.jboss.pnc.dto.DeliverableAnalyzerOperation storedDTO = all.getContent().iterator().next();
        assertThat(storedDTO.getStartTime()).isEqualTo(operation.getStartTime().toInstant());

        // Update the same operation (recreated from scratch), must not be saved as a new one; and startTime must not
        // change (only endTime and status can change)
        Date now = new Date();
        Map<String, String> operationParameters = new HashMap<String, String>();
        operationParameters.put("url-0", "https://github.com/project-ncl/pnc/archive/refs/tags/2.1.1.tar.gz");
        operationParameters.put("url-1", "https://github.com/project-ncl/pnc-common/archive/refs/tags/2.1.0.zip");
        DeliverableAnalyzerOperation sameOperation = DeliverableAnalyzerOperation.Builder.newBuilder()
                .id(operation.getId())
                .startTime(now)
                .endTime(now)
        ...
        org.jboss.pnc.dto.DeliverableAnalyzerOperation newOperationDTO = delAnalyzerOperationMapper
                .toDTO(sameOperation);

        newOperationDTO = provider.saveOrUpdate(newOperationDTO.getId(), newOperationDTO);
        all = provider.getAll(0, 10, null, null);
        assertThat(all.getContent()).hasSize(1)
                .haveExactly(
                        1,
                        new Condition<>(op -> sameOperation.getId().getId().equals(op.getId()), "Operation present"));
        org.jboss.pnc.dto.DeliverableAnalyzerOperation newStoredDTO = all.getContent().iterator().next();
        assertThat(newStoredDTO.getStartTime()).isNotEqualTo(now.toInstant());
        assertThat(newStoredDTO.getEndTime()).isEqualTo(now.toInstant());
        assertThat(newStoredDTO.getStatus()).isEqualTo(OperationStatus.SUCCESSFUL);
        assertThat(newStoredDTO.getStartTime()).isEqualTo(storedDTO.getStartTime());
    ...
    public void testGetSpecificOperation() {
        ...
        assertThat(specific.getStatus()).isEqualTo(operation.getStatus());
    }
