...
public class ProductMilestoneProviderImpl extends
        ...
        Map<String, Map<String, List<ArtifactVersion>>> artifactsDeliveredInMilestonesMap = tuples.stream()
                .collect(
                        Collectors.groupingBy(
                                tuple -> parseArtifactNameFromTuple(tuple),
    ...
    private String parseArtifactNameFromTuple(Tuple tuple) {
        String deployPath = tuple.get(2, String.class);
        RepositoryType repositoryType = tuple.get(3, RepositoryType.class);
    ...
    private Stream<Map.Entry<String, ArrayList<ArtifactVersion>>> parseMilestonePresencesFromTuple(
            Tuple tuple,
        ...
        return milestoneIds.stream().filter(milestoneId -> {
            Integer milestonePresence = tuple.get(4 + milestoneIds.indexOf(milestoneId), Integer.class);
            return milestonePresence == 1;
        }).map(milestoneId -> Map.entry(milestoneId.toString(), Lists.newArrayList(artifactVersion)));
    }
