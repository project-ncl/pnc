...
public class ProductMilestoneProviderImpl extends
    ...
    private org.jboss.util.graph.Graph<ProductMilestone> createMilestoneInterconnectionGraph(
            String milestoneId,
            Integer depthLimit) {
        org.jboss.util.graph.Graph<ProductMilestone> graph = new org.jboss.util.graph.Graph<>();
        UndirectedGraphBuilder<ProductMilestone, String> graphBuilder = new UndirectedGraphBuilder<>(
                id -> getSpecific(id),
                node -> {
                    var tuples = milestoneRepository
                            .getMilestonesSharingDeliveredArtifacts(Integer.valueOf(node.getId()));
                    return parseMilestoneInterconnectionGraphNeighborsFromTuples(tuples);
                });
    ...
    private List<VertexNeighbor<String>> parseMilestoneInterconnectionGraphNeighborsFromTuples(List<Tuple> tuples) {
