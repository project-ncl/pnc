...
public interface RepositoryConfigurationEndpoint{
    ...
    @Operation(summary = "Gets all Repository Configurations",
            responses = {
    ...
    @Operation(summary = "Creates a new Repository Configuration. NOTE: it does not create the repository in the scm server, it only creates an entry in PNC database. Useful when you already have internal and external scm repositories created.",
            responses = {
    ...
    @POST
    public Response createNew(RepositoryConfiguration repositoryConfiguration);

    @Operation(summary = "Gets a specific Repository Configuration",
            responses = {
                @ApiResponse(responseCode = SUCCESS_CODE, description = SUCCESS_DESCRIPTION,
                    content = @Content(schema = @Schema(implementation = RepositoryConfigurationSingleton.class))),
    ...
    public Response getSpecific(
            @Parameter(description = "Repository Configuration id", required = true) @PathParam("id") Integer id);

    @Operation(summary = "Updates an existing Repository Configuration",
            responses = {
                @ApiResponse(responseCode = SUCCESS_CODE, description = SUCCESS_DESCRIPTION),
                @ApiResponse(responseCode = INVALID_CODE, description = INVALID_DESCRIPTION,
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    ...
    @PUT
    @Path("/{id}")
    public Response update(@Parameter(description = "Repository Configuration id", required = true) @PathParam("id") Integer id,
            RepositoryConfiguration repositoryConfiguration);

    @Operation(summary = "The matching is done using LIKE.",
            responses = {
                @ApiResponse(responseCode = SUCCESS_CODE, description = SUCCESS_DESCRIPTION,
                    content = @Content(schema = @Schema(implementation = RepositoryConfigurationPage.class))),
                @ApiResponse(responseCode = NO_CONTENT_CODE, description = NO_CONTENT_DESCRIPTION,
    ...
    public Response search(@Parameter(description = PAGE_INDEX_DESCRIPTION) @QueryParam(PAGE_INDEX_QUERY_PARAM) @DefaultValue(PAGE_INDEX_DEFAULT_VALUE) int pageIndex,
            ...
            @Parameter(description = "Url part to search for", required = true) @QueryParam(SEARCH_QUERY_PARAM) String scmUrl);


    @Operation(summary = "Only exact matches are returned.",
            responses = {
                @ApiResponse(responseCode = SUCCESS_CODE, description = SUCCESS_DESCRIPTION,
                    content = @Content(schema = @Schema(implementation = RepositoryConfigurationPage.class))),
                @ApiResponse(responseCode = NO_CONTENT_CODE, description = NO_CONTENT_DESCRIPTION,
    ...
    public Response match(@Parameter(description = PAGE_INDEX_DESCRIPTION) @QueryParam(PAGE_INDEX_QUERY_PARAM) @DefaultValue(PAGE_INDEX_DEFAULT_VALUE) int pageIndex,
            @Parameter(description = PAGE_SIZE_DESCRIPTION) @QueryParam(PAGE_SIZE_QUERY_PARAM) @DefaultValue(PAGE_SIZE_DEFAULT_VALUE) int pageSize,
            @Parameter(description = SORTING_DESCRIPTION) @QueryParam(SORTING_QUERY_PARAM) String sort,
            @Parameter(description = "Url to search for", required = true) @QueryParam(SEARCH_QUERY_PARAM) String scmUrl);
