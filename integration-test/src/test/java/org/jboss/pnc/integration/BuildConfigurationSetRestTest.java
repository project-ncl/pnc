...
public class BuildConfigurationSetRestTest {
    ...
    public void prepareBaseData() {
        ...
        ValidatableResponse responseProd = given().contentType(ContentType.JSON).port(getHttpPort()).when().get(PRODUCT_REST_ENDPOINT).then().statusCode(200)
        ...
        Response responseBuildConf = given().contentType(ContentType.JSON).port(getHttpPort()).when()
    ...
    public void testCreateNewBuildConfSet() throws IOException {
        ...
        Response response = given().body(buildConfSetTemplate.fillTemplate()).contentType(ContentType.JSON)
